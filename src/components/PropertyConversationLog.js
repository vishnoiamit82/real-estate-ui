...
const PropertyConversationLog = ({ propertyId }) => {
    ...
    const handleMarkAsUnread = async (conversationId) => {
        try {
            await axiosInstance.post(`${process.env.REACT_APP_API_BASE_URL}/property-conversations/${conversationId}/mark-as-unread`);
            const refreshed = await axiosInstance.get(`${process.env.REACT_APP_API_BASE_URL}/property-conversations/${propertyId}`);
    ...
    const handleMarkAsRead = async (conversationId) => {
        try {
            await axiosInstance.post(`${process.env.REACT_APP_API_BASE_URL}/property-conversations/${conversationId}/mark-as-read`);
            const refreshed = await axiosInstance.get(`${process.env.REACT_APP_API_BASE_URL}/property-conversations/${propertyId}`);
            setLogs(refreshed.data);
        } catch (err) {
                ...
                <div className="space-y-4">
                    {logs.map((log, idx) => (
                        <div key={idx} className="border-b pb-3">
                            ...
                            {/* Mark as unread */}
                            {log.type === 'reply' && log.isRead && (
                                <button
                                    onClick={() => handleMarkAsUnread(log._id)}
